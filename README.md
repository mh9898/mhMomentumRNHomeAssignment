...
The files inside the **app** directory. This project uses [file-based routing](https://docs.expo.dev/router/introduction).

## Demo
https://github.com/user-attachments/assets/5f240f99-98fb-441d-92e8-4a08f4f056f6
...
- Uses responsive text utilities for consistent font sizing across devices

- **AppText** (`components/AppText.tsx`): Reusable text component with responsive font sizing

- Automatically applies responsive font sizing based on screen size
- Font size decreases by 0.6x on screens smaller than 845px height
- Supports `adjustsFontSizeToFit` and `minimumFontScale` props for text fitting
- Processes style props to apply responsive font sizing automatically
- Used throughout the app for consistent text rendering across different screen sizes
- Theme-aware styling support

- **AppTextLines** (`components/AppTextLines.tsx`): Text component with responsive line height

- Automatically adjusts line height based on screen size
- Tighter line height (18px) on smaller screens (< 845px height)
- Default line height (34px) on larger screens
- Supports custom line height override
- Includes `adjustsFontSizeToFit` and `minimumFontScale` for better text fitting
- Used in `AppTitle` and `ScreenTitle` components for consistent typography
- Used in `PrivacyStatement` component for multi-line text display
...
- Theme-aware styling
- Uses `AppText` component for error messages with responsive sizing
- Uses responsive text utilities for consistent font sizing across devices
...
- Uses `AppText` component for responsive text rendering
...
- Uses `AppText` component and responsive text utilities for consistent sizing
...
- Theme-aware styling
- Uses `AppTextInput` component with responsive text utilities for consistent sizing
...
- Uses `AppTextLines` component for responsive line height
...
- Uses `AppTextLines` component for responsive line height and text fitting
- Uses responsive text utilities for font sizing on small screens
...
- **Responsive Text** (`utils/responsiveText.ts`): Responsive text sizing utilities for cross-device compatibility
- `normalize(size)`: Normalizes font size based on screen width (base width: 375px)
  - Ensures consistent scaling across different device widths
  - Rounds to nearest pixel for crisp rendering
- `responsiveFontSize(baseSize)`: Returns responsive font size that decreases by 0.6x on small screens
  - Small screen threshold: height < 845px
  - Automatically adjusts font size for better readability on smaller devices
- `responsiveLineHeight(fontSize, baseMultiplier, minMultiplier)`: Calculates responsive line height
  - Tighter line height on smaller screens for better text fitting
  - Configurable multipliers for fine-tuning
- `isSmallScreen()`: Checks if current screen height is less than 845px
  - Helper function for conditional responsive behavior
- `normalizeFont(size)`: Combines width-based normalization with automatic small screen adjustment
  - Applies 0.6x scaling on small screens before normalizing
  - One-step solution for responsive font sizing
- `normalizeResponsive(size, smallScreenSize)`: Advanced normalization with optional small screen override
  - Allows custom font size for small screens
  - Combines width and height-based scaling
- Used throughout the app in components like `AppText`, `AppTextInput`, `AppButton`, `PaymentForm`, and more
- Ensures consistent typography and readability across all device sizes
